"""
Cayley-aligned comparison of uniform vs. Cayley initialization in IPR-MPNN.

This script:
1. Calculates the optimal number of virtual nodes based on Cayley graph structure
2. Creates two versions of the MUTAG dataset config
3. Trains IPR-MPNN models with both initializations
4. Compares the performance metrics and generates visualizations
"""
...
def create_modified_config(edge_init_type, avg_nodes_per_graph, output_path, epochs=100, num_runs=5):
    ...
    # Calculate the optimal number of virtual nodes based on Cayley structure
    if edge_init_type == 'cayley':
        num_centroids = calculate_optimal_virtual_nodes(avg_nodes_per_graph)
        logger.info(f"For {avg_nodes_per_graph} base nodes, calculated {num_centroids} optimal virtual nodes for Cayley initialization")
    else:
        # For uniform initialization, we'll use the same number as Cayley for fair comparison
        num_centroids = calculate_optimal_virtual_nodes(avg_nodes_per_graph)
        logger.info(f"For uniform initialization, using {num_centroids} virtual nodes to match Cayley")
    
    # Add edge_init_type and adjust other parameters
    config['edge_init_type'] = edge_init_type
    config['scorer_model']['num_centroids'] = num_centroids
    config['wandb'] = {
...
def visualize_initialization_patterns(avg_nodes_per_graph, output_dir):
    """Visualize the weight patterns for both initialization methods."""
    ...
    # Calculate optimal number of virtual nodes based on Cayley structure
    num_virtual_nodes = calculate_optimal_virtual_nodes(avg_nodes_per_graph)
    
    logger.info(f"Creating visualizations for {avg_nodes_per_graph} base nodes and {num_virtual_nodes} virtual nodes")
    
    # Initialize with both methods
    uniform_weights = uniform_init_weights(avg_nodes_per_graph, num_virtual_nodes)
    cayley_weights = cayley_initialize_edge_weight(avg_nodes_per_graph, num_virtual_nodes)
    ...
    with open(os.path.join(output_dir, f"statistics_{timestamp}.txt"), "w") as f:
        ...
        f.write("==================================\n\n")
        f.write(f"Base Nodes: {avg_nodes_per_graph}\n")
    ...
    logger.info(f"Statistics saved to {output_dir}/statistics_{timestamp}.txt")
    return uniform_stats, cayley_stats, num_virtual_nodes
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='cayley_aligned_comparison', help='Output directory')
    parser.add_argument('--force-run', action='store_true', help='Force rerun experiments even if logs exist')
    parser.add_argument('--visualize-only', action='store_true', help='Only visualize initialization patterns without running experiments')
    parser.add_argument('--avg-nodes', type=int, default=17, help='Average number of nodes per graph in MUTAG dataset')
    ...
    # Calculate the optimal number of virtual nodes based on the average graph size
    avg_nodes_per_graph = args.avg_nodes
    
    # Step 1: Visualize the initialization patterns
    vis_dir = os.path.join(args.output_dir, "visualizations")
    uniform_stats, cayley_stats, num_virtual_nodes = visualize_initialization_patterns(
        avg_nodes_per_graph, vis_dir
    )
    
    if args.visualize_only:
        logger.info("Visualization complete. Skipping experiments as --visualize-only is set.")
        return
    
    # Step 2: Create modified configs for IPR-MPNN experiments
    logger.info(f"Creating configs with {num_virtual_nodes} virtual nodes")
    ...
    # Step 3: Run experiments if needed
