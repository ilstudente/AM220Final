...
if __name__ == "__main__":
    ...
    # Plot timing
    plt.figure(figsize=(10, 6))
    plt.plot(sizes, [r["timing"]["uniform_init_time"] for r in results], 'o-', label='Uniform')
    plt.plot(sizes, [r["timing"]["cayley_init_time"] for r in results], 'o-', label='Cayley')
    plt.title('Initialization Time')
    plt.xlabel('Number of Base Nodes')
    plt.ylabel('Time (seconds)')
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{save_path}/timing_comparison.png")
    plt.close()
    
    # Plot bottleneck ratio
    plt.figure(figsize=(10, 6))
    plt.plot(sizes, [r["properties"]["bottleneck_ratio"]["uniform"] for r in results], 'o-', label='Uniform')
    plt.plot(sizes, [r["properties"]["bottleneck_ratio"]["cayley"] for r in results], 'o-', label='Cayley')
    plt.title('Bottleneck Ratio')
    plt.xlabel('Number of Base Nodes')
    plt.ylabel('Ratio (higher means less bottleneck)')
    plt.legend()
    plt.grid(True)
    plt.savefig(f"{save_path}/bottleneck_comparison.png")
    plt.close()
    
    print(f"Comparison plots saved to {save_path}/")

def main():
    print("Advanced Cayley vs. Uniform Initialization Comparison")
    ...
    for case in test_cases:
        result = run_comparison(
            num_base_nodes=case["base"],
            num_virtual_nodes=case["virtual"],
        ...
        print(f"\nStatistics for {case['base']} base nodes and {case['virtual']} virtual nodes:")
        print("Uniform Initialization:")
        for key, value in result["statistics"]["uniform"].items():
            print(f"  {key}: {value}")
        
        print("\nCayley Initialization:")
        for key, value in result["statistics"]["cayley"].items():
            print(f"  {key}: {value}")
    
    # Plot comparison metrics
    plot_comparison_metrics(all_results, output_dir)
    
    # Save all results
    with open(f"{output_dir}/all_results_{timestamp}.json", "w") as f:
        json.dump(all_results, f, indent=2)
    
    print(f"\nAll results saved to {output_dir}/all_results_{timestamp}.json")
