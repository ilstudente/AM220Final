"""
Modified version of IPRMPNNModel with enhanced Cayley initialization.
"""
...
class EnhancedIPRMPNNModel(nn.Module):
    """
    Enhanced IPR-MPNN model with learnable edge weights between base nodes and virtual nodes.
    Uses enhanced Cayley initialization that ensures all edges have non-zero weights.
    """
    def __init__(self, input_dim, hidden_dim, output_dim, max_virtual_nodes=None, edge_init_type='uniform', top_k=None):
        super(EnhancedIPRMPNNModel, self).__init__()
        ...
        for graph_idx in range(num_graphs):
            ...
            if graph_id not in self.graph_edge_weights:
                ...
                if self.edge_init_type == 'cayley':
                    # Use enhanced Cayley graph initialization with non-zero values for all edges
                    init_weights = enhanced_cayley_initialize_edge_weight(
                        num_base_nodes=num_nodes, 
                        num_virtual_nodes=num_virtual_nodes,
                        cayley_n=cayley_n,
                        high_value=5.0,  # Higher value for Cayley edges
                        low_value=0.1,   # Small positive value for non-Cayley edges
                        verbose=False
                    ).to(device)
                else:
            ...
            if self.top_k is not None and self.top_k < num_virtual_nodes:
                # Use our enhanced top-k function to ensure exactly k connections per node
                edge_weights = force_topk_connections(edge_weights, k=self.top_k)
            else:
